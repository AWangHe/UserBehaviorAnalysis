...
object OrderTimeout {
  def main(args: Array[String]): Unit = {
    ...
    //3.把模式应用到stream上面，得到一个pattern stream
    val patternStream = CEP.pattern(orderEventStream, orderPayPattern)

    //4.调用select 方法，提取事件序列,超时的时间要做报警提示
    val orderTimeuotOutputTag = new OutputTag[OrderResult]("orderTimeOut")

    val resultStream = patternStream.select(orderTimeuotOutputTag, new OrderTimeoutSelect(), new OrderPaySelect())

    resultStream.print("payed")
    resultStream.getSideOutput(orderTimeuotOutputTag).print("timeout")
