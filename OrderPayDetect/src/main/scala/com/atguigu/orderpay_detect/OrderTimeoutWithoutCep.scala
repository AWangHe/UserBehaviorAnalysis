...
class OrderPayMatch() extends  KeyedProcessFunction[Long, OrderEvent, OrderResult]{
  ...
  //保存定时器的事件戳为状态
  lazy val timerState: ValueState[Long] = getRuntimeContext.getState(new ValueStateDescriptor[Long]("timer-state", classOf[Boolean]))
  ...
  override def processElement(value: OrderEvent, ctx: KeyedProcessFunction[Long, OrderEvent, OrderResult]#Context, out: Collector[OrderResult]): Unit = {
    ...
    if(value.eventType == "create"){
      ...
      if(timerTs > 0){
        ...
        //2.2 pay 先到了，更新状态，注册定时器等待create
...
class OrderTimeoutWarning() extends  KeyedProcessFunction[Long, OrderEvent, OrderResult]{
  ...
  //保存pay 是否来过的状态
  lazy val isPayedSate: ValueState[Boolean] = getRuntimeContext.getState(new ValueStateDescriptor[Boolean]("ispayed-state", classOf[Boolean]))

  override def processElement(value: OrderEvent, ctx: KeyedProcessFunction[Long, OrderEvent, OrderResult]#Context, out: Collector[OrderResult]): Unit = {
    //先取出状态标识
    val isPayed: Boolean = isPayedSate.value()
