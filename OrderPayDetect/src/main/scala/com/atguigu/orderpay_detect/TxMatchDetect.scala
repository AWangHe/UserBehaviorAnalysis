...
object TxMatchDetect {
  ...
  class TxPayMatch() extends  CoProcessFunction[OrderEvent, ReceiptEvent, (OrderEvent, ReceiptEvent)]{
    ...
    lazy val payState: ValueState[OrderEvent] = getRuntimeContext.getState("payState", classOf[OrderEvent])
    lazy val receiptState: ValueState[ReceiptEvent] = getRuntimeContext.getState("receipt-state", classOf[ReceiptEvent])
